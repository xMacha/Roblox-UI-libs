local discord = "https://discord.gg/d3rE8G8S"
...
-- System ESPa-- Sprawdzamy czy zmienna już istnieje (czy skrypt był odpalony)
if getgenv().XydeScriptLoaded then
    game.StarterGui:SetCore("SendNotification", {
        Title = "Xyde Script",
        Text = "Skrypt jest już włączony!",
        Duration = 5
    })
    return -- To słowo kluczowe zatrzymuje dalsze wykonywanie skryptu
end

-- Jeśli nie był, ustawiamy zmienną na true
...
local function UpdateESP()
    ...
    -- 3. Evidence (Fingerprints, Orbs, EMF)
    if ESP_Settings.Evidence then
        ...
        for _, v in pairs(EvidenceFolder.EMF:GetChildren()) do if v.Name == "EMF5" then Highlight(v, "EMF 5", Color3.fromRGB(255, 255, 0)) end end
    else
...
-- ============================================================================
-- 3. INTERFEJS https://discord.gg/d3rE8G8S
-- ============================================================================
local Window = Rayfield:CreateWindow({
   ...
   ShowText = "Xyde",
      ...
      Invite = "d3rE8G8S", -- The Discord invite code, do not include discord.gg/. E.g. discord.gg/ ABCD would be ABCD
...
local GhostRoomLabel = TabMain:CreateLabel("Ghost Room: --")
...
TabMain:CreateButton({
    ...
    Callback = function()
        ...
        Rayfield:Notify({Title="Scanning", Content="Starting accurate scan..."})
            ...
            for _, r in pairs(Map.Rooms:GetChildren()) do
                if r:IsA("Folder") and r:FindFirstChild("Hitbox") then
                    ...
                    if EMFTool.Color == Color3.fromRGB(131, 156, 49) then
                        task.wait(1.0) -- Podwójne sprawdzenie dla dokładności
                        if EMFTool.Color == Color3.fromRGB(131, 156, 49) then
                            return r
                        end
                    end
...
-- Automatyczne sprawdzanie Motion (Pętla)
task.spawn(function()
    while task.wait(0.5) do
        ...
        if Motions then
            for _, m in pairs(Motions:GetDescendants()) do
                if m:IsA("Part") then
                    if m.Color == Color3.fromRGB(252, 52, 52) then -- Czerwony
                        foundMotion = true
                        break
                    end
                end
        ...
        if foundMotion then
            ...
            MotionLabel:Set("Motion: Not Found")
        end
    end
end)

-- PRZYCISKI DO SPRAWDZANIA DOWODÓW (Bez Motion Button)
TabMain:CreateButton({
    ...
    Callback = function()
        ...
        if tempeture and tempeture < 0 then
            Rayfield:Notify({Title="Evidence", Content="Freezing Found!", Image="snowflake"})
...
TabVisuals:CreateToggle({Name="Evidence ESP (Prints/Orbs)", Callback=function(v) ESP_Settings.Evidence = v end})
